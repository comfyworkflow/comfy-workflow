...
plus one ``setup-pillar1.bat`` orchestrator into ``setup-windows/``.
...
``setup-pillar1.bat`` is a thin orchestrator that runs
``01-install-base.bat`` then ``install-sdxl.bat``. Supports
``--unattended`` flag (or ``COMFY_NONINTERACTIVE`` env var) for SSH
dispatch (DA-013 compatible — never launches ComfyUI interactively).
...
ORCHESTRATOR_NAME: str = "setup-pillar1.bat"
