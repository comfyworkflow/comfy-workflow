@echo off
REM ============================================================================
REM Comfy Workflow - Z-Image Turbo (bf16 + fp8 E4M3FN)
REM ============================================================================
...
REM ============================================================================
REM Pillars: #6
REM Models (from models_manifest.yaml): z_image_turbo_bf16, z_image_turbo_fp8, z_image_shared_encoder, flux_shared_vae
REM Hardware minimo: 32 GB RAM, 8 GB VRAM
REM Total download (this script): ~25.00 GiB
REM ============================================================================
setlocal EnableDelayedExpansion

set "REPO_RAW=https://raw.githubusercontent.com/comfyworkflow/comfy-workflow/main"
set "CATEGORY=Image\ZImage"
...
echo.
echo ========================================================
echo  Comfy Workflow - Z-Image Turbo (bf16 + fp8 E4M3FN)
echo ========================================================
echo  Pillars: #6
echo  Hardware minimo: 32 GB RAM, 8 GB VRAM
echo  Total download: ~25.00 GiB
echo ========================================================
echo.
...
echo [1/3] Downloading model files (total: ~25.00 GiB)...
call :download_or_skip "https://huggingface.co/Comfy-Org/z_image_turbo/resolve/main/split_files/diffusion_models/z_image_turbo_bf16.safetensors" "C:\ComfyUI_windows_portable\ComfyUI\models\diffusion_models\z_image_turbo_bf16.safetensors" 12309866400
call :download_or_skip "https://huggingface.co/Kijai/Z-Image_comfy_fp8_scaled/resolve/main/z-image-turbo_fp8_scaled_e4m3fn_KJ.safetensors" "C:\ComfyUI_windows_portable\ComfyUI\models\diffusion_models\z_image_turbo_fp8_e4m3fn.safetensors" 6158115074
call :download_or_skip "https://huggingface.co/Comfy-Org/z_image_turbo/resolve/main/split_files/text_encoders/qwen_3_4b.safetensors" "C:\ComfyUI_windows_portable\ComfyUI\models\text_encoders\qwen_3_4b.safetensors" 8044982048
...
echo [2/3] Pruning legacy flat-layout workflow files (z_image_*.json)...
call :cleanup_flat_workflows "z_image_*.json"

echo [3/3] Shipping 2 workflow file(s)...
call :ship_workflow "z_image_turbo_bf16.json"
call :ship_workflow "z_image_turbo_fp8.json"
...
echo =========================================================================
echo.
echo   Models installed: bf16 (12.3 GB) + fp8 E4M3FN (6.16 GB) + Qwen3-4B encoder (8.04 GB) + FLUX VAE (0.3 GB) - total ~26.8 GB
echo.
echo   Workflows in ComfyUI sidebar:
echo     Comfy Workflow ^> Image ^> ZImage ^> (bf16 + fp8 E4M3FN)
echo.
echo   Default recommendation: z_image_turbo_fp8.json (6 GB DiT fits modest cards natively, no offload)
echo   For reference precision: z_image_turbo_bf16.json (12 GB DiT, uses offload on 8-12 GB cards)
echo   8-step distilled, CFG 1.0 - no negative prompt (ConditioningZeroOut zeros the positive).
echo   Single Qwen3-4B encoder (bf16 full precision, constant across both tiers) - isolates DiT precision.
